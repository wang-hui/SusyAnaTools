...
int SearchBins::find_Binning_Index(int ibJet, int iTop, double MT2, double met, double ht) const
{
    ...
    {
      double mt2ht=MT2;
      if (iTop>=3 || ibJet>=3) mt2ht=ht;
      if(searchBins_[iBin].compare(ibJet, iTop, mt2ht, met))
      {
   return iBin;
      }
    }
...
void SearchBins::drawSBregionDef(const double ymin_Yields, const double ymax_Yields, const bool logscale, const bool drawOnlyLines){
    ...
    if( !drawOnlyLines ){
       if(logscale)
       {
          ...
          if(NSB == 84)
          {
             ...
             ttext_ntop->DrawLatex(79.5 + adjHalfBin, ymax_Yields/5000. ,"N_{t} #geq 3");
          }
          else if(NSB == 59)
          {
   ...
   if(NSB == 84)
       ...
       ttextmt2->DrawLatex( 3.0, ymax_Yields/50000. , "M_{T2}=[200,300]");
            ttextmt2->DrawLatex( 11.0, ymax_Yields/25000. , "M_{T2}=[400, 550]");
