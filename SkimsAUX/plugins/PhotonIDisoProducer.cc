  ...
  debug(iConfig.getUntrackedParameter<bool>("debug",true))
{
  ...
  //Andres Gamma Tlorentz                                                                                                                    
  gammaSrc_ = iConfig.getParameter<edm::InputTag>("gammaSrc");
  GammaSrc_Tok_ = consumes<std::vector<pat::Photon> >(gammaSrc_);
...
PhotonIDisoProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  //Andres
  std::auto_ptr<std::vector<TLorentzVector> > gammaLVec(new std::vector<TLorentzVector>());
  std::auto_ptr< std::vector< double > > photon_pt( new std::vector< double > () );
  std::auto_ptr< std::vector< double > > photon_eta( new std::vector< double > () );
  std::auto_ptr< std::vector< double > > photon_phi( new std::vector< double > () );
  iEvent.getByToken(GammaSrc_Tok_, gamma);

  for(unsigned int ip = 0; ip < gamma->size(); ip++){
    TLorentzVector perGammaLVec;
    //if(gamma->at(ip).pt() > 100){
      perGammaLVec.SetPtEtaPhiE( gamma->at(ip).pt(), gamma->at(ip).eta(), gamma->at(ip).phi(), gamma->at(ip).energy() );
      //}    
  ...
  iEvent.put(photon_hadronization , "hadronization" );

  //Andres
  iEvent.put(photon_pt , "photonPt" );
  iEvent.put(photon_eta , "photonEta" );
  iEvent.put(photon_phi , "photonPhi" );
  //Gamma TLorentz
  iEvent.put(gammaLVec,"gammaLVec");
}
